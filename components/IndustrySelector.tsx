...
import { IndustryNode, INDUSTRY_TREE_DATA } from '../src/constants/industries';
...
export const IndustrySelector: React.FC<Props> = ({ initialSelected, onConfirm, onClose }) => {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set(initialSelected.filter(id => id !== '全部')));
  ...
  const renderNode = (node: IndustryNode, level = 0) => {
    const hasChildren = node.children && node.children.length > 0;
    const isExpanded = expandedIds.has(node.id);
    // In this case, ID is the same as the name for selection mapping if needed, or we just map by name.
    // The previous implementation used strings like "房屋建筑业"
    // Let's use node.name as the selection key to match 'moreFilters.industryClassification'
    const isSelected = selectedIds.has(node.name);
  ...
  return (
    <div className="fixed inset-0 bg-white z-[200] flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="flex items-center justify-between px-2 h-12 border-b border-gray-100 flex-shrink-0 pt-safe bg-white shadow-sm z-10">
        ...
        <h2 className="text-[17px] font-bold text-gray-900">行业分类</h2>
        <div className="w-10"></div>
      </div>
      
      {/* Content */}
      <div className="flex-1 overflow-y-auto bg-white">
        {INDUSTRY_TREE_DATA.map(node => renderNode(node, 0))}
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 pb-[calc(env(safe-area-inset-bottom)+12px)] bg-white flex-shrink-0">
        ...
        <button 
          onClick={() => onConfirm(selectedIds.size > 0 ? Array.from(selectedIds) : ['全部'])}
